...
def download(inputCfg):    
    if os.path.isfile(("{}/run_list_MC.txt").format(inputCfg["input"]["output_dir_name"])):
        os.system(("rm {}/run_list_MC.txt").format(inputCfg["input"]["output_dir_name"]))

    fOut = open(("{}/run_list_MC.txt").format(inputCfg["input"]["output_dir_name"]), 'x')
    ...
    #sorted_runs = sorted(inputCfg["input"]["run_list_MC"])
    #print("Sorted runs:", sorted_runs)
    
    print("----- Download and save files in %s -----" % (inputCfg["input"]["output_dir_name"]))
    for iRun in range(0, len(inputCfg["input"]["run_list_MC"])):
        
        file_type = inputCfg["input"]["file_type"]
        run = inputCfg["input"]["run_list_MC"][iRun]
        alien_path = inputCfg["input"]["alien_input_path"][iRun]

        os.system("mkdir -p %s/%s" % (output_dir, run))
        
        os.system("alien_cp alien://%s/AnalysisResults.root file:%s/%s/AnalysisResultsMC_run_%s.root" % (alien_path, output_dir, run, run))
...
def merge(inputCfg):
    ...
    runs = inputCfg["input"]["run_list_MC"]
    for run in runs:
...
def main():
    ...
    parser.add_argument('cfgFileName', metavar='text', default='config_MC.yml', help='config file name')
