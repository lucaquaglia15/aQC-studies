// ------------ Data and MC comparison (eta, phi and pT distributions) + ratios MC/data(A02D level) + quality flags for CBT_muon 
#include <iostream>
#include <TAttLine.h>
#include <TAxis.h>
#include <TCanvas.h>
...
void Data_MC_comparison_run_cbt_muon(string MC_prod, string period, string apass, int run){
  ...
  hOutGood_thresholds2.open(Form("%s/goodRuns_thresholds_sigmas_cbt_muon.txt",output_dir_name[0]), std::ios_base::app);

  ofstream hOutBad_thresholds2;
  hOutBad_thresholds2.open(Form("%s/badRuns_thresholds_sigmas_cbt_muon.txt",output_dir_name[0]), std::ios_base::app);
